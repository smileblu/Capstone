...
6. recommended: true는 payback_months(투자 회수기간)가 가장 짧고 reduction_rate 합산이 0.05 이상인
   시나리오 하나에만 표시합니다. 중소기업은 자금 여력이 제한적이므로 조기 회수 가능성이 최우선입니다.
   회수기간이 동일하면 investment_cost_krw가 낮은 시나리오를 선택합니다.
   일반적으로 Moderate(A)가 해당됩니다.
...
- risk_assessment:         K-ETS 데이터 블록의 수치만 사용. 연간 환산 비용 반드시 포함.
                           K-ETS 직접 규제 여부는 반드시 "사업장 단위 25,000 tCO₂e 기준"으로 서술.
                           미달 시: "본 사업장의 연간 환산 배출량은 약 NNN tCO₂e로, 사업장 단위 25,000tCO₂e
                           기준에는 미달합니다. 다만 K-ETS 적용 여부는 업체 전체 배출량 및 사업장별 배출량을
                           종합해 판단해야 합니다." 형식으로 서술. (NNN은 제공된 연간 환산 배출량 수치로 대체)
...
- scenario_recommendation: 투자 대비 탄소 감축 효율(원/kgCO₂e) 관점에서 시나리오를 비교.
                           회수기간이 가장 짧은(recommended=true) 시나리오를 명확히 권고.
                           회수기간이 100년 이상인 경우 회수기간은 언급하지 말고 감축 효율 중심으로 서술.
- conclusion:              추천 시나리오(recommended=true)를 반드시 명시하여 단기 실행 방향을 제시."""
...
def _build_report_prompt(req: CompanyReportRequest) -> str:
    ...
    def sc_line(s: Optional[_ReportScenario], label: str) -> str:
        ...
        cost_krw = round(s.co2_reduction_kg / 1000 * kets_price)
        return (f"- {label} ({s.name}): 절감 {s.co2_reduction_kg:.1f}kg, "
                f"절감비용 {cost_krw:,}원(K-ETS {kets_price:,}원/t 기준), 투자 {s.investment_cost_krw:,}원")
    ...
    kets_exemption_text = (
        f"사업장 단위 25,000 tCO₂e 기준 미달 (본 사업장 연간 환산 배출량 약 {annual_ton:.1f} tCO₂e). "
        f"다만 K-ETS 적용 여부는 업체 전체 배출량 및 사업장별 배출량을 종합해 판단해야 합니다."
        if exemption
        else f"규제 대상 검토 필요 (본 사업장 연간 환산 배출량 약 {annual_ton:.1f} tCO₂e — 25,000 tCO₂e 이상)"
    )
    ...
    # 추천 시나리오 (LLM에게 명시하여 표-결론 일치 유도)
    rec_sc = next((s for s in req.scenarios if s.recommended), req.scenarios[0] if req.scenarios else None)
    rec_info = f"{rec_sc.id} ({rec_sc.name})" if rec_sc else "A"
    ...
    return (
        ...
        f"{sc_line(sb, 'B')}\n"
        f"{sc_line(sc, 'C')}\n\n"
        f"★ 추천 시나리오 (recommended=true): {rec_info}\n"
        f"  scenario_recommendation과 conclusion에서 반드시 {rec_info} 시나리오를 권고 전략으로 명시하세요.\n\n"
        f"아래 JSON으로 출력:\n"
        f"{{\n"
...
def _build_esg_pdf(req: CompanyReportRequest, llm_text: dict, ts: str) -> str:
    ...
    t4 = Table([
        ["항목","수치"],
        ["보고기간 탄소 비용 (3개월 합산)", f"{ed.cost_total_krw:,}원"],
        ["연간 환산 탄소 비용",            f"약 {_cost_annual:,}원"],
        ["연간 환산 배출량",               f"약 {_annual_ton:.1f} tCO₂e"],
        ["K-ETS 직접 규제",
         f"사업장 단위 기준 미달 (연간 {_annual_ton:.1f} tCO₂e)\n업체 전체 기준 별도 확인 필요"
         if _exemption else f"규제 대상 검토 필요 (연간 {_annual_ton:.1f} tCO₂e)"],
        ["K-ETS 단가 기준",    f"{_base_date} KRX KAU25 종가"],
        ["향후 6개월 누적 예상 비용", f"{round(six_cost):,}원"],
        ["K-ETS 단가",         f"{k_ets:,}원/tCO₂e"],
    ...
    def sc_v(s: Optional[_ReportScenario], field: str) -> str:
        ...
        # cost_saving_krw는 BE에서 12,000원/ton 기준으로 계산됨 → 보고서 K-ETS 단가(k_ets)로 재산출
        if field == 'cost_saving_krw':
            co2_kg = getattr(s, 'co2_reduction_kg', 0) or 0
            return f"{round(co2_kg / 1000 * k_ets):,}"
